    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 pt-4 pb-10">
      ...
      <Kanban>
        {TRACKER_COLUMNS.map((column) => (
          <TrackerColumn column={column} kanban={kanban} key={column} />
        ))}
      </Kanban>
    </div>
  );
    ...
    <ContextMenu>
      ...
      <ContextMenu.Popover>
        <ContextMenu.Menu>
          ...
          <ContextMenu.Separator />
          <ContextMenu.Section>
            <ContextMenu.Item onAction={() => kanban.removeItem(taskId)} textValue="Delete">
              <TrashBin />
    ...
    <div className="flex flex-col gap-2 p-3">
      ...
      <div className="mt-0.5 flex -space-x-2">
        {task.assignees.slice(0, 3).map((assignee) => (
          <Avatar className="ring-background size-5 ring-2" key={assignee.name} size="sm">
