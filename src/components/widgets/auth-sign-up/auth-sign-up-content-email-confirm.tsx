...
export function AuthSignUpContentEmailConfirm() {
  ...
  return (
    <form className="flex w-full flex-col gap-8" onSubmit={form.handleSubmit(onSubmit)}>
      <div className="flex flex-col items-center gap-2 text-center">
        <h1 className="text-2xl font-semibold tracking-tight">Confirme o e-mail comercial</h1>
        <p className="text-muted text-sm leading-relaxed">
          Insira o código de 6 dígitos enviado para
        </p>
        <AuthSignUpChangeDestination
          actionLabel="Alterar e-mail"
          hint="Errou o e-mail ou não tem acesso a ele? Altere para receber o código em outro endereço."
          onEdit={() => goToStep(1)}
          value={data.email}
        />
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <p className="text-muted text-center text-xs leading-relaxed">
          ...
          {countdown > 0 ? (
            <span className="text-accent">Reenviar em {countdown}s</span>
          ) : (
            <Button
